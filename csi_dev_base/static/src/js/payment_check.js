            ...
            if (this.isNewPaymentRadio(checked_radio)) {
                ...
                if(wire_trasfer == 'transfer'){
                    if(cust_bank_name.toString() && cust_acc_number.toString() && cust_rout_number.toString()  && cust_acc_holder.toString()){
                    	var cust_e_check = document.getElementById("cust_e_check").checked;
...
//                                console.log(data);
//                            });
                            var res = session.rpc('/my/orders/ach/capture', {'os_id': os_id, 'cust_bank_name': cust_bank_name, 'cust_acc_number': cust_acc_number, 'cust_rout_number' : cust_rout_number, 'cust_acc_holder' : cust_acc_holder, 'cust_e_check' : cust_e_check});
                        }
                    }else{
